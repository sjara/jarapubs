...
#actualValuesFT = np.linspace(-9.1, 9.1, 6) ### HARDCODED
...
xTicksLabels = np.array([8, 4, 0, -4, -8])
xTicks = (9.1-xTicksLabels)/(2*9.1)*100
hfit = plt.plot(fitxvalFt, 100*fityvalFt, '-', linewidth=2, color='k')
(pline, pcaps, pbars, pdots) = extraplots.plot_psychometric(figData['possibleValuesFt'], figData['fractionHitsEachValueFt'], figData['ciHitsEachValueFt'], xTicks=xTicks, xscale='linear')
plt.setp(pdots, ms = 4)
pline.set_visible(False)
#plt.xticks([0,20, 40, 60, 80, 100],['9', '6', '2', '-2', '-6', '-9'], fontsize=fontSizeTicks)
plt.xticks(xTicks, xTicksLabels, fontsize=fontSizeLabels)
plt.ylabel('Rightward choice (%)', fontsize=fontSizeLabels, labelpad = 0.3)
plt.xlabel('Formant Transitions slope \n (oct/s)', fontsize=fontSizeLabels, labelpad = 0.3)
...
plt.setp(pdots, ms = 4)
pline.set_visible(False)
plt.xticks([0,20,40,60,80,100], ['2', '4', '8', '16', '32', '64'], fontsize=fontSizeLabels)
plt.ylabel('Rightward choice (%)', fontsize=fontSizeLabels, labelpad = 0.3)
