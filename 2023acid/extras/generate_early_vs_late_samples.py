...
nSamples = 40
medianOEIsalineEachStimLastPeriod = np.empty((nSamples, len(studyparams.STIMULI)))
medianOEIdoiEachStimLastPeriod = np.empty((nSamples, len(studyparams.STIMULI)))
...
for indperiod, celldb in enumerate([celldbAll, celldbEarly, celldbLate, celldbLate]):
    ...
    for inds, stim in enumerate(studyparams.STIMULI):
        ...
        OEIsalineOrig = OEIsaline.copy()
        OEIdoiOrig = OEIdoi.copy()
        
        # -- Match saline OEI for late period to that of early period --
        if indperiod==3:
            for randomSeed in range(nSamples): # [10]: #
                OEIsaline = OEIsalineOrig.copy()
                OEIdoi = OEIdoiOrig.copy()
                ...
                newOEI = OEIsaline[newOEIind].copy()
                ...
                #medianOEIsaline = np.mean(OEIsaline)
                #medianOEIdoi = np.mean(OEIdoi)
                posEffect = '*' if medianOEIsaline>medianOEIdoi else ''
                print(f'Median saline OEI: {medianOEIsaline:0.3f} \t'+
                      f'Median DOI OEI: {medianOEIdoi:0.3f} \t {posEffect}')
                medianOEIsalineEachStimLastPeriod[randomSeed, inds] = medianOEIsaline
                medianOEIdoiEachStimLastPeriod[randomSeed, inds] = medianOEIdoi
...
doiEffectDirection = medianOEIsalineEachStimLastPeriod > medianOEIdoiEachStimLastPeriod
print('')
print(f'Percentage of samples with OEIsaline > OEIdoi (last 3 days matched): {np.mean(doiEffectDirection):0.1%}')
