...
for indm, metric in enumerate(metrics):
    ...
    thisMetricPre = celldb['pre'+metric][selectedCells]
    thisMetricSaline = celldb['saline'+metric][selectedCells]
    thisMetricDOI = celldb['doi'+metric][selectedCells]
    eachMetricPre.append(thisMetricPre)
    eachMetricSaline.append(thisMetricSaline)
    eachMetricDOI.append(thisMetricDOI)
...
thisAx = plt.subplot(gsMetrics[0])
MIwidth = studyutils.modulation_index(eachMetricDOI[1], eachMetricSaline[1])
MImaxchange = studyutils.modulation_index(eachMetricDOI[2], eachMetricSaline[2])
#MIwidth = studyutils.modulation_index(eachMetricPre[1], eachMetricSaline[1])
#MImaxchange = studyutils.modulation_index(eachMetricPre[2], eachMetricSaline[2])
if 1:
    plt.plot(np.log2(eachMetricSaline[1]), MImaxchange, '.')
    plt.xlabel('Saline log(Width)')
    plt.ylabel('MI Max Δ')
else:
    plt.plot(MImaxchange, MIwidth, '.')
    plt.axis('equal')
    plt.xlabel('MI Max Δ')
    plt.ylabel('MI Width')
plt.show()

#corrcoeff = np.corrcoef(MImaxchange, MIwidth)
corrcoeff, pvalCorr = stats.pearsonr(MImaxchange, MIwidth)
print(f'Correlation (MImaxchange,MIwidth): {corrcoeff:0.3f} (p={pvalCorr:0.4f})')

corrcoeff, pvalCorr = stats.pearsonr(eachMetricSaline[1], MImaxchange)
#corrcoeff, pvalCorr = stats.spearmanr(eachMetricSaline[1], MImaxchange)
print(f'Correlation (Saline Width, MI Max Δ): {corrcoeff:0.3f} (p={pvalCorr:0.4f})')

corrcoeff, pvalCorr = stats.pearsonr(eachMetricSaline[0], MImaxchange)
#corrcoeff, pvalCorr = stats.spearmanr(eachMetricSaline[0], MImaxchange)
print(f'Correlation (Saline BF, MI Max Δ): {corrcoeff:0.3f} (p={pvalCorr:0.4f})')
