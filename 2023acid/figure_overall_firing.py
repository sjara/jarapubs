...
metrics = ['ToneBaselineFiringRate', 'ToneAvgEvokedFiringRate']
metricLabels = ['Baseline firing', 'Avg evoked firing']
metricLims = [[0, 60], [0, 90]]
...
wstatRunPreDOI, pValRunPreDOI = stats.wilcoxon(fractionRunning[0], fractionRunning[1])
...
plt.figtext(labelPosX[0], labelPosY[1], 'C', fontsize=fontSizePanel, fontweight='bold')
for indp, plabel in enumerate(['B','D','E']):
...
for indrun, runningCond in enumerate(runningConditions):
    ...
    for indm, metric in enumerate(metrics):
        ...
        print(f'[N={nCells}] {metric} Median : saline={medianSaline:0.4f}, ' +
              f'DOI={medianDOI:0.4f}   p={pVal:0.8f}')
        ...
        medianMI1 = np.nanmedian(modIndex1)
        medianMI2 = np.nanmedian(modIndex2)
        print(f'[N={nCells}] {metric} MI: {medianMI1:0.3f}, {medianMI2:0.3f} (p = {pValModInd:0.6f})')
