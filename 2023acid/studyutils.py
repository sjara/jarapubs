...
def find_tone_responsive_cells(celldb, frThreshold=10, allreagents=True):
    """
    ...
    alpha = 0.05/N_FREQ
    if allreagents:
        responsive = ( (celldb['preToneResponseMinPval'] < alpha) &
                       (celldb['preToneFiringRateBestFreq'] > frThreshold) &
                       (celldb['salineToneResponseMinPval'] < alpha) &
                       (celldb['salineToneFiringRateBestFreq'] > frThreshold) &
                       (celldb['doiToneResponseMinPval'] < alpha) &
                       (celldb['doiToneFiringRateBestFreq'] > frThreshold) )
    else:
        responsive = ( (celldb['preToneResponseMinPval'] < 0.05/16) &
                       (celldb['preToneFiringRateBestFreq'] > frThreshold) )
    return responsive
...
def find_freq_selective(celldb, minR2=0.05):
    ...
    """
    freqSelective = (celldb['preToneSelectivityPval']<0.05)
    goodFitPre = (celldb['preToneGaussianRsquare'] > minR2)
    goodFitSaline = (celldb['salineToneGaussianRsquare'] > minR2)
    return freqSelective & goodFitPre & goodFitSaline
    
    
def find_good_gaussian_fit(celldb, minR2=0.05):
    """
    ...
    goodFitPre = (celldb['preToneGaussianRsquare'] > minR2)
    goodFitSaline = (celldb['salineToneGaussianRsquare'] > minR2)
    return goodFitPre & goodFitSaline
...
def load_running_data():
    ...
    for subject in studyparams.SUBJECTS:
        runningData[subject] = dict(np.load(os.path.join(dbPath, f'running_{subject}.npz')))
    return runningData
